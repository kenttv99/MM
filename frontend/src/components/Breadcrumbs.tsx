...
const Breadcrumbs: React.FC = () => {
  ...
  const getBreadcrumb = (segment: string, index: number) => {
    ...
    if (index === 0 && !isEventSlug) {
      ...
      const cachedTitle = localStorage.getItem(`event-title-${eventId}`);
      const cachedSlug = localStorage.getItem(`event-slug-${eventId}`);
      href = `/event/${cachedSlug || segment}`;
      label = cachedTitle || `ID: ${eventId}`;
    }
