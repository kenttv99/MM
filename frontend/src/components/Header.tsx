...
const AvatarDisplay = ({ avatarUrl, fio, email }: { avatarUrl?: string; fio?: string; email: string }) => {
  ...
  if (avatarUrl && !imgError) {
    return (
      <Image
        ...
        width={40}
        height={40}
        className="w-10 h-10 rounded-full object-cover hover:opacity-90 sm:w-8 sm:h-8"
  ...
  return (
    <div className="w-10 h-10 sm:w-8 sm:h-8 bg-orange-100 rounded-full flex items-center justify-center text-orange-500 text-xl font-bold hover:bg-orange-200">
    ...
    <>
      <header
        ...
        className={`fixed top-0 left-0 right-0 z-30 transition-all duration-300 ${
          isScrolled ? "bg-white/95 backdrop-blur-sm shadow-lg py-2 sm:py-3" : "bg-white/90 py-3 sm:py-4"
        }`}
      >
        <div className="container mx-auto px-4 sm:px-6 flex items-center justify-between">
          ...
          <div className="flex items-center md:hidden">
            {isAuth && userData && (
              <Link href="/profile" className="mr-4 text-orange-500 hover:text-orange-600 z-40">
                <AvatarDisplay avatarUrl={userData.avatar_url} fio={userData.fio} email={userData.email} />
              </Link>
            )}
            <button
              onClick={toggleMobileMenu}
              className="text-gray-700 hover:text-orange-500 p-2 z-40 relative w-10 h-10 flex flex-col justify-center items-center"
              ...
              <div className="w-6 h-6 relative flex justify-center items-center">
                <motion.span
                  ...
                  animate={{
                    ...
                    translateY: isMobileMenuOpen ? 0 : -5,
                  }}
          ...
          <div className="hidden md:flex items-center space-x-4">
            {isAuth && userData ? (
              <>
                <Link
                  href="/partner"
                  className="text-orange-500 hover:text-orange-600 px-4 py-2 rounded-lg hover:bg-orange-50 text-sm sm:text-base"
                >
                  Стать партнером
                </Link>
                ...
                <button
                  ...
                  className="text-orange-500 hover:text-orange-600 ml-2 p-2"
                  title="Выход"
                >
                  <svg className="h-6 w-6 sm:h-5 sm:w-5" viewBox="0 0 20 20" fill="currentColor">
                    <path
              ...
              <div className="flex items-center space-x-4">
                <motion.button
                  ...
                  className="px-4 py-2 border border-orange-500 text-orange-500 rounded-lg hover:bg-orange-50 text-sm sm:text-base"
                >
                  Регистрация
                </motion.button>
                <motion.button
                  ...
                  whileTap={{ scale: 0.97 }}
                  onClick={openLogin}
                  className="px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 text-sm sm:text-base"
                >
                  Войти
                </motion.button>
              </div>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-white/95 backdrop-blur-md z-50 md:hidden flex flex-col overflow-hidden"
          >
            <div className="flex justify-between items-center p-4 border-b border-gray-200 bg-white">
              <Link href="/" className="transition-transform duration-300 hover:scale-105" onClick={toggleMobileMenu}>
                <Logo />
              </Link>
              <button
                onClick={toggleMobileMenu}
                ...
                >
                  {item.onClick ? (
                    <button
                      ...
                      className="w-full py-3 text-gray-800 hover:text-orange-500 transition-colors duration-200 text-xl font-medium focus:outline-none"
                    >
                      {item.label}
                    </button>
                  ) : (
                    <Link
                      href={item.href || "#"}
                      className="block w-full py-3 text-gray-800 hover:text-orange-500 transition-colors duration-200 text-xl font-medium"
